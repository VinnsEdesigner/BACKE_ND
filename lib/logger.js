 ...
 *
 * Dual-output logger: stdout (container logs) + /app/logs/app.log (file).
 * HF Spaces container logs = stdout only — file-only logging = invisible.
 * Every log level writes to both simultaneously.
 */
...
// ── ANSI color codes for stdout readability ───────────────────────────────────
const COLORS = {
  reset:  '\x1b[0m',
  debug:  '\x1b[36m',   // cyan
  info:   '\x1b[32m',   // green
...
function parseLine(line) {
  ...
  try {
    ...
    return { timestamp: null, level: 'raw', namespace: 'log', message: trimmed, meta: {}, pid: process.pid, host: os.hostname() };
  }
}

// ── STDOUT FORMATTER ──────────────────────────────────────────────────────────
// Pretty-prints each log entry to container stdout so HF Spaces shows it.
function formatForStdout(entry) {
  const ts        = entry.timestamp ? entry.timestamp.slice(11, 23) : '??:??:??.???'; // HH:MM:SS.mmm
...
// ── CORE LOG FUNCTION ─────────────────────────────────────────────────────────
// Writes to BOTH stdout AND file. This is the critical fix.
async function log(level, namespace, message, meta = {}) {
  ...
  // ── stdout (container logs — immediate, synchronous) ──────────────────────
  // Must write to stdout even if file write fails.
...
function child(name) {
  ...
  return {
    ...
    child(subName) { return child(`${namespace}:${subName}`); },
  };
}
