...
try {
  ...
  // If logManager fails to load (syntax error, truncated file, etc.),
  // fall back to a minimal in-memory stub so the process can continue.
  // We intentionally avoid throwing here because logger is used during
  // application startup and should not take the whole app down.
  try { console.error('[logger] failed to load logManager:', err); } catch { /* ignore */ }

  logManager = {
    // redactString should accept non-strings safely
    redactString: (input) => (typeof input === 'string' ? input : String(input)),
    // minimal async log that no-ops
    log: async () => undefined,
  };
}
