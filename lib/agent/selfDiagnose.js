...
// selfDiagnose.js
// Analyzes structured log entries and produces typed findings.
// Never emits SSE directly — SSE wiring lives in server.js or monitor bridge.
// All detector failures are caught — diagnosis never crashes the process.
...
function safeLower(input) {
  ...
  if (typeof input === 'object') {
    try { return JSON.stringify(input).toLowerCase(); } catch { return ''; }
  }
  return String(input).toLowerCase();
}
...
// FIX: sha1 → sha256 (no deprecation warning)
...
function detectRateLimit(entry) {
  ...
  if (
    ...
    return baseFinding({
      ...
      evidence: { provider, status, message: entry?.message, meta: entry?.meta },
    });
  }
...
function detectProviderDown(entry) {
  ...
  if (
    hasAny(search, [
      'all_providers_down', 'provider down', 'marked down',
      'unavailable', 'service unavailable', 'health check failed',
      'down until',
    ])
  ) {
    return baseFinding({
...
function detectBuildFailure(entry) {
  ...
  if (
    hasAny(search, [
      'build failed', 'buildfailure', 'esbuild failed',
      'cannot find module', 'module not found',
      'syntaxerror', 'referenceerror', 'typeerror',
      'npm err!', 'rollup failed', 'vite failed', 'webpack failed',
    ])
  ) {
    return baseFinding({
...
function detectMemoryPressure(entry) {
  ...
  const rssMb  = pickNestedNumber(entry?.meta, [
    'rssMb', 'rss_mb', 'memoryMb', 'memory_mb', 'heapUsedMb',
  ]) || null;

  if (
    (rssMb !== null && rssMb >= DEFAULT_MEMORY_WARNING_RSS_MB) ||
    hasAny(search, [
      // FIX: removed bare 'rss' — too broad, matches 'process', 'express'
      // Use specific JSON key patterns instead
      '"rss":', 'rss_mb', 'rss_bytes',
      'heap out of memory', 'heapused',
      'heap total', 'memory pressure',
      'out of memory', 'memory leak',
    ])
  ) {
    const severity = (rssMb !== null && rssMb >= DEFAULT_MEMORY_CRITICAL_RSS_MB)
      ? 'critical'
      : 'warning';
    ...
    return baseFinding({
      ...
      evidence: { rssMb, message: entry?.message, meta: entry?.meta },
    });
  }
...
function detectSseFailure(entry) {
  ...
  if (
    hasAny(search, [
      'sse failure', 'eventsource failed', 'heartbeat timeout',
      'heartbeat missed', 'missed pong', 'reconnect failed',
      'broadcast failed', 'stream disconnected',
      'connection closed', 'orphaned process', 'sse disconnected',
    ])
  ) {
    return baseFinding({
...
function detectDatabaseIssue(entry) {
  ...
  if (
    hasAny(search, [
      'supabase', 'postgres', 'database error',
      'relation does not exist', 'foreign key',
      'unique violation', 'timed out',
      'connection refused', 'deadlock',
...
function detectGitIssue(entry) {
  ...
  if (
    hasAny(search, [
      'non-fast-forward', 'rejected', 'merge conflict',
      'permission denied', 'protected branch',
      'failed to push', 'could not resolve host', 'remote rejected',
    ])
  ) {
    return baseFinding({
      category: 'git_issue',
...
function detectFileSystemIssue(entry) {
  ...
  if (
    hasAny(search, [
      ...
      'path too long', 'is a directory', 'not a directory',
    ])
  ) {
    return baseFinding({
...
function detectAuthIssue(entry) {
  ...
  if (
    hasAny(search, [
      ...
      'authentication failed', 'missing auth',
    ])
  ) {
    return baseFinding({
      category: 'auth_issue',
...
function createLiveMonitor(options = {}) {
  const {
    onFinding = null,
    // FIX: renamed onBatch → onAnalysis (batch implies multiple entries, misleading)
    onAnalysis = null,
    onEntry    = null,
...
function toBroadcastCard(finding) {
  ...
  return {
    type:
      finding.severity === 'critical' ? SSE.EVENT_TYPES.WARNING  :
      finding.severity === 'warning'  ? SSE.EVENT_TYPES.FINDING  :
