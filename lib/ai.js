...
async function mistralGap() {
  const wait = AGENT.MISTRAL_GAP_MS
    ? AGENT.MISTRAL_GAP_MS - (Date.now() - lastMistralCall)
    : 1000 - (Date.now() - lastMistralCall);
...
const health = {
  ...
  mistral_chat:   { status: PROVIDER_STATUS.OK, downUntil: 0 }, // devstral + large + lean
...
// ── SCHEMA NORMALIZER ─────────────────────────────────────────────────────────
// Converts any provider response into { text, model, tokens_used }
...
async function callGemini(modelKey, modelName, messages, maxTokens) {
...
// ── WATERFALL ─────────────────────────────────────────────────────────────────
// Provider order matches BUILD_LAWS model waterfall exactly

async function complete(options = {}) {
  const {
    messages,
    maxTokens    = AGENT.MAX_TOKENS,
    preferCode   = false,   // true → start with devstral instead of groq
    systemPrompt = null,
  } = options;
  ...
  // ── 1. Groq llama-3.3-70b (primary brain) ──────────────────────────────────
  if (!preferCode && isUp('groq')) {
    try {
      ...
      logger.warn('ai', 'Groq failed → trying next', { error: err.message });
    }
  }

  // ── 2. Mistral devstral-medium (code writing / preferCode entry) ────────────
  if (isUp('mistral_chat')) {
    try {
      ...
      logger.warn('ai', 'Devstral failed → trying next', { error: err.message });
    }
  }
  ...
  if (isUp('mistral_chat')) {
    try {
      ...
      logger.warn('ai', 'Mistral-large failed → trying next', { error: err.message });
    }
  }
  ...
  if (isUp('gemini_lite')) {
    try {
      logger.debug('ai', 'Trying gemini-flash-lite');
      ...
      logger.warn('ai', 'Gemini flash-lite failed → trying next', { error: err.message });
    }
  }

  // ── 5. Gemma 26B MoE (fast last resort, 1.5K RPD) ──────────────────────────
  if (isUp('gemma_26b')) {
    try {
      ...
      logger.warn('ai', 'Gemma-26b failed → trying next', { error: err.message });
    }
  }
  ...
  if (isUp('gemma_31b')) {
    try {
      ...
      logger.warn('ai', 'Gemma-31b failed → trying next', { error: err.message });
    }
  }

  // ── 7. Gemini 2.5 flash (heavy tasks — 20 RPD, absolute last resort) ────────
  if (isUp('gemini')) {
    try {
  ...
  logger.error('ai', 'All providers down — cannot complete request');
  throw new Error('all_providers_down');
}

// ── STREAMING ─────────────────────────────────────────────────────────────────
// Streams chunks via callback. Falls back to complete() if provider doesn't stream.
...
async function stream(options = {}, onChunk) {
  ...
  // Only Groq supports streaming in current setup — others fall back to complete()
  if (isUp('groq')) {
    try {
